...
// Nhớ giữ import cn nếu mày đang dùng nó ở đâu đó, nếu không thì bỏ đi cũng được
...
export const DealFlowSection = () => {
    return (
        // GIỮ NGUYÊN CSS NỀN TỐI Ở NGOÀI
        <section className="relative w-full bg-kizuna-primary py-24 md:py-32 overflow-hidden">
            <div className="mx-auto max-w-6xl px-6 lg:px-8">

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">

                    {/* CỘT TRÁI: GIỮ NGUYÊN 100% NHƯ CŨ */}
                    <div className="lg:col-span-5 lg:sticky lg:top-32 flex flex-col">
                        <motion.span
                            initial={{ opacity: 0, y: 10 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            className="font-geist text-[10px] md:text-xs font-bold uppercase tracking-[0.25em] text-zinc-500 mb-6"
                        >
                            Deal-Flow Nổi Bật
                        </motion.span>
                        ...
                        <motion.p
                            ...
                            className="font-inter text-zinc-400 text-base md:text-lg leading-relaxed mb-8"
                        >
                    ...
                    {/* CỘT PHẢI: CHỈ THAY ĐỔI RIÊNG PHẦN CARD (WHITE BENTO) */}
                    <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {TOP_PROJECTS.map((project, index) => (
                            <motion.div
                                ...
                                // Thẻ Card được chuyển sang nền trắng, shadow để nổi bật trên nền tối
                                className="group flex flex-col bg-white border border-[#102c1e]/5 rounded-[1.5rem] p-5 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
                            >
                                {/* Ảnh Thumbnail dự án */}
                                <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden mb-5 bg-slate-100">
                                    ...
                                    />
                                    {/* Dải Tags chìm nền trắng chữ đen */}
                                    <div className="absolute top-3 left-3 flex gap-2">
                                        {project.tags.map(tag => (
                                            <span
                                                key={tag}
                                                className="px-2.5 py-1 rounded-md bg-white/90 backdrop-blur-md border border-[#102c1e]/5 font-geist text-[10px] font-bold text-[#102c1e] uppercase tracking-wider shadow-sm"
                                            >
                                                {tag}
                                            </span>
                                        ))}
                                ...
                                <div className="flex justify-between items-start mb-3">
                                    {/* Tiêu đề dùng font-outfit */}
                                    <h3 className="font-outfit text-xl font-bold text-[#102c1e] group-hover:text-[#4a7c5f] transition-colors">
                                        {project.title}
                                    </h3>
                                    {/* Tên founder dùng font-inter */}
                                    <span className="font-inter text-[11px] font-medium text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100 mt-0.5">
                                        by {project.founder}
                                    </span>
                                </div>

                                {/* Mô tả dùng font-inter */}
                                <p className="font-inter text-sm text-slate-500 line-clamp-2 mb-6">
                                    {project.desc}
                                </p>

                                {/* Các chỉ số (Metrics) dùng font-geist & font-mono */}
                                <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                                    {project.metrics.map((metric, idx) => (
                                        <div key={idx} className="flex flex-col">
                                            <span className="font-geist text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                                                {metric.label}
                                            </span>
